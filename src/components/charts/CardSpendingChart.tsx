...
export function CardSpendingChart({
  data,
}: {
  data: { name: string; bank: string; value: number }[];
}) {
  if (data.length === 0) {
    return (
      <p className="py-10 text-center text-sm text-black/50 dark:text-white/50">
        Sem gastos no cartão ainda.
      </p>
    );
  }
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={data} dataKey="value" nameKey="name" innerRadius="55%" outerRadius="100%" stroke="none">
              {data.map((d, i) => (
                <Cell key={i} fill={bankInfo(d.bank).color} />
              ))}
            </Pie>
          ...
          <li key={d.name} className="flex items-center justify-between gap-2">
            <span className="flex min-w-0 items-center gap-1.5">
              <span
                ...
                style={{ backgroundColor: bankInfo(d.bank).color }}
              />
